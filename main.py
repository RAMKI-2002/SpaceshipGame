...
def draw_window(red,yellow,red_bullets,yellow_bullets,red_health,yellow_health):
    ...
    WIN.blit(YELLOW_SPACESHIP,(yellow.x,yellow.y)) #images comes under surface , so to display surfaces on window we use blit
    WIN.blit(RED_SPACESHIP,(red.x,red.y)) #images comes under surface , so to display surfaces on window we use blit
...
def handle_movement_yellow(keys_pressed,yellow):
    if keys_pressed[pygame.K_a] and yellow.x - VEL > 0: # LEFT
        yellow.x -= VEL
    if keys_pressed[pygame.K_d] and yellow.x + yellow.width + VEL < MIDDLE_BORDER.x: # RIGHT
        yellow.x += VEL
...
def main():
    red = pygame.Rect(800,300,SPACESHIP_WIDTH,SPACESHIP_HEIGHT) # (x,y,width,height)
    ...
    while run:
        clock.tick(FPS) #this while loop runs 60(FPS) times per second
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LCTRL and len(yellow_bullets) < MAX_BULLETS:
                    bullet = pygame.Rect(yellow.x + yellow.width, yellow.y + yellow.height//2+1, 10, 5)
                    yellow_bullets.append(bullet)
                    BULLET_FIRE_SOUND.play()

                if event.key == pygame.K_RCTRL and len(red_bullets) < MAX_BULLETS:
                    bullet = pygame.Rect(red.x, red.y + red.height//2+1, 10, 5)
